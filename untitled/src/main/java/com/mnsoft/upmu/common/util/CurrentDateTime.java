...
/**
 * 유형 설명을 삽입하십시오. 작성 날짜: (2000-11-01 오후 4:08:37)
 *
 * @작성자: Administrator
 */
public final class CurrentDateTime {
	...
	public synchronized static int getMaxday(String yyyymm) {
		...
		if (new_mm == 1)
			...
			if (((new_yyyy % 4 == 0) && (new_yyyy % 100 != 0))
					|| (new_yyyy % 400 == 0))
				total_day = 29;
			else
				total_day = 28;
	...
	public static String getMonth(String yyyymmdd, int addMonth) {
	...
	/**
	 * 메소드 설명을 삽입하십시오. 작성 날짜: (2002-08-09 오후 6:11:52)
	 *
	...
	/**
	 * 시작일과 종료일의 날짜 차이를 계산한다. 작성 날짜: (2006-11-08)
	 *
	 * @return long
	 * @param type
	 *            java.lang.String 시작일 ( 20061201 )
	 * @param type
	 *            java.lang.String 종료일 ( 20061221 )
	 */
	...
	/**
	 * num을 주면 ...7을 7일후날짜..-7 일전 날짜... 0 날짜일 1요일
	 */
	public static String[][] getDayOfDay(String YYYYMMDD, int num) {
		...
		try {
			...
			if (Stdt.length() == 6)
				Stdt = Stdt + "01"; // 1날. YYYYMM
			else if (Stdt.length() < 6)
				Stdt = CurrentDateTime.getDate(); // ""
			else if (Stdt.length() > 8)
				Stdt = Stdt.substring(0, 8); // YYYYMMDDHHMM
				...
				for (i = 0; i < loop_cnt; i++) {
					...
					if (++day > last_day) {
						...
						if (month == 12) {
							...
							++month;
						day = 1;
						Stdt = String.valueOf(year) + (month < 10 ? "0" : "")
								+ month;
						last_day = getMonthDays(Stdt);
					}
				}
	...
	/**
	 * yyyymm1 과 yyyymm2 사의 월수를 리턴. ex> 200112, 200203 --> 4 s_yearmm 은
	 * e_yyyymm보다 작거나 같아야 한다.
	 *
	 */
	...
	/**
	 * yyyymmdd1 과 yyyymmdd2 사의 일수를 리턴. ex> 20011201, 20020301 --> 61 s_ymd 은
	 * e_ymd보다 작거나 같아야 한다.
	 *
	 */
	...
	public static int getDateGugan(String s_ymd, String e_ymd) {
		...
		while (!frdt.equals(endt)) {
			...
			endt = String.valueOf(year)
					+ (String.valueOf(month).length() != 2 ? "0"
							+ String.valueOf(month) : String.valueOf(month))
					+ (String.valueOf(day).length() != 2 ? "0"
	...
	/**
	 * 이전/년월 구하기 ex> 200207, -1 -> 200206을 리턴
	 */
	...
	/**
	 * 년도를 Combo로 가져온다. 작성 날짜: (2006-11-21 오전 10:24:34)
	 *
	 * @return String
	 */
	...
	/**
	 * 년도를 Combo로 가져온다. 작성 날짜: (2008-04-07 오전 17:24:34)
	 *
	 * @return String
	 */
	...
	/**
	 * 월을 Combo로 가져온다. 작성 날짜: (2006-11-21 오전 10:24:34)
	 *
	 * @return String
	 */
	...
	public static String getCustomDate(String str){
		String rtn = str.replaceAll("-", "");
		String mm = rtn.substring(4, 6);
		String dd = rtn.substring(6, 8);
		return mm + "/" + dd;
	}
	...
	public static String getTimeFormat(String str){
	 ...
	 public static String getMilScnd() {
    ...
    public static List<String> getFromBetweenToYmd(String s1, String s2) throws ParseException {
    ...
    /** * 년 월 일 날짜 더하기 * * @param dt(날짜) , y(년) , m(월), d(일) * @Exam addDate("20180910",1,12,1) -->20200911 * @return String */ 
