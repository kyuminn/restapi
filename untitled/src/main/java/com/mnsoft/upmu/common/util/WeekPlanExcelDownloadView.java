...
public class WeekPlanExcelDownloadView extends AbstractExcelView {
	...
	XSSFCellStyle dateCellFormat5; // lightYellow
...
//		SXSSFSheet sheet = createSheet(workBook, "Sheet1");
		...
		dateCellFormat5.setFillForegroundColor(new XSSFColor(new Color(255, 250, 153)));
			...
			if(methodName.contains("&")) {
				...
				for(String nameValue : nameSplit) {
					if(splitCnt == 0) {
						...
						method3 = cls.getMethod(nameSplit[splitCnt]);
					}else if(splitCnt == 3){
						method4 = cls.getMethod(nameSplit[splitCnt]);
					}
					splitCnt++;
				}
				...
				Object val3 = method3.invoke(vo);
				Object val4 = method4.invoke(vo);
				
				SXSSFCell cell = row.createCell(col, SXSSFCell.CELL_TYPE_STRING);
				
				
				if(methodVal.startsWith("getW")) {
					if(val2.toString().equals("M") && val3.toString().equals("0")) {
						if(val4.toString().equals("00:00")) {
				...
				}else if(methodVal.startsWith("getSum_W")){
					cell.setCellStyle(dateCellFormat2);
				}
				...
				if (val == null) {
					...
					if(methodName.toString().equals("getAverage")) {
						cell.setCellStyle(dateCellFormat2);
					} else if(methodName.toString().startsWith("getSum_w")) {
						cell.setCellStyle(dateCellFormat2);
					} else if(methodName.toString().startsWith("getSum_t")) {
						cell.setCellStyle(dateCellFormat4);
					} else {
						cell.setCellStyle(dateCellFormat);
					}
