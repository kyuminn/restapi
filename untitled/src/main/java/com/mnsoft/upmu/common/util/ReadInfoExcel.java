						...
						if (cell == null) {
							...
							case XSSFCell.CELL_TYPE_NUMERIC:
								...
								if(DateUtil.isCellDateFormatted(cell)) {
									...
									cellStr = new SimpleDateFormat("yyyyMMdd").format(date);
								} else {
									String strtemp = new BigDecimal(String.format("%.2f", cell.getNumericCellValue())).stripTrailingZeros().toPlainString();
									if(strtemp.indexOf("9999999999") > -1) {
										strtemp = new BigDecimal(String.format("%.2f", cell.getNumericCellValue())).stripTrailingZeros().toPlainString();
									}
									cell.setCellType(cell.CELL_TYPE_STRING);
							...
							case XSSFCell.CELL_TYPE_BOOLEAN:
								result.put(excelKeyArr[colIndex], Boolean.toString(cell.getBooleanCellValue()));
								break;
